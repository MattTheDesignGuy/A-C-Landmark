...
export default function ServicesPage() {
  return (
    <>
      {/* Hero Banner */}
      <section className="relative bg-navy pt-32 pb-20 lg:pb-28">
            ...
            <section key={service.title} className={`py-16 lg:py-24 ${i > 0 ? 'border-t border-light-grey' : ''}`}>
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center ${isEven ? '' : 'lg:[direction:rtl]'}`}>
                  {/* Image placeholder */}
                  <div className={`aspect-[4/3] rounded-lg overflow-hidden ${isEven ? '' : 'lg:[direction:ltr]'}`}>
                    <div className="w-full h-full bg-gradient-to-br from-navy/80 to-navy-light flex items-center justify-center">
                      <span className="font-heading text-3xl text-gold/40 font-bold text-center px-4">
                        {service.title}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className={isEven ? '' : 'lg:[direction:ltr]'}>
                    ...
                    <Link
                      to="/contact"
                      className="inline-flex items-center text-gold font-semibold text-lg hover:gap-3 gap-2 transition-all"
                    >
